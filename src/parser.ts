// Parser Module - Parses different input types
...
export class Parser {
  ...
  private parsePRD(content: string): ParsedResult {
    ...
    // Simple approach: find the section and extract flows from it
  ...
  private inferAction(description: string): string {
    ...
    if (desc.includes('输入') || desc.includes('填写') || desc.includes('录入') || desc.includes('填写')) {
      return 'input';
    }
    if (desc.includes('点击') || desc.includes('选择') || desc.includes('触发')) {
      return 'click';
    }
    if (desc.includes('提交') || desc.includes('确认') || desc.includes('完成') || desc.includes('完成注册')) {
      return 'submit';
    }
    if (desc.includes('验证') || desc.includes('检查') || desc.includes('校验') || desc.includes('系统验证')) {
      return 'validate';
    }
    ...
    if (desc.includes('进入') || desc.includes('跳转') || desc.includes('跳转首页')) {
      return 'navigate';
    }
  ...
  private extractInputs(description: string): FlowInput[] {
    ...
    if (desc.includes('账号') || desc.includes('用户名') || desc.includes('手机号') || desc.includes('手机') || desc.includes('邮箱') || desc.includes('注册方式')) {
      inputs.push({
        name: 'account',
        type: 'text',
        required: true,
    ...
    if (desc.includes('选择登录方式') || desc.includes('选择注册方式')) {
      inputs.push({
        name: 'method',
        type: 'select',
        required: true,
